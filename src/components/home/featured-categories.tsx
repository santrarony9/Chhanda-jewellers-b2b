            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {categories.map((cat, index) => (
                        <motion.div
                            key={index}
                            className={`${cat.colSpan} h-[450px] group relative overflow-hidden border border-white/5 bg-surface-dark backdrop-blur-sm hover:border-primary-500/30 transition-all duration-700`}
                            initial={{ opacity: 0, y: 20 }}
                            whileInView={{ opacity: 1, y: 0 }}
                        ...
                        >
                            <Link href={cat.href} className="flex flex-col h-full justify-between p-8 z-10 relative">
                                {/* Gradient Background */}
                                <div className={`absolute inset-0 bg-gradient-to-br ${cat.gradient} opacity-20 group-hover:opacity-40 transition-opacity duration-700`} />
                                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-90" />

                                {/* Visual Cue */}
                                <div className="absolute inset-0 opacity-50 group-hover:opacity-70 transition-opacity duration-700 -z-10 bg-blend-overlay">
                                    {/* Placeholder for noise/texture if visually needed */}
                                </div>
                                <div className="absolute top-0 right-0 w-full h-full opacity-60 group-hover:opacity-80 transition-all duration-1000 transform group-hover:scale-110 -z-10 origin-center">
                                    {cat.visual ? (
                                        <img src={cat.visual} alt={cat.title} className="w-full h-full object-cover object-center grayscale-[30%] group-hover:grayscale-0 transition-all duration-700" />
                                    ) : (
                                ...
                                <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-x-4 group-hover:translate-x-0">
                                    <div className="bg-primary-500 text-black p-3 rounded-none shadow-[0_0_15px_rgba(212,175,55,0.4)]">
                                        <ArrowRight className="h-5 w-5" />
                                    </div>
                                </div>

                                <div className="transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                                    <h3 className="text-3xl font-serif text-white mb-3 group-hover:text-primary-100 transition-colors duration-300 drop-shadow-md">{cat.title}</h3>
                                    <div className="h-[1px] w-12 bg-primary-500/50 mb-3 group-hover:w-24 transition-all duration-700 ease-out" />
                                    <p className="text-gray-300 text-sm tracking-widest uppercase font-medium">{cat.subtitle}</p>
                                </div>
                            </Link>
                        </motion.div>
                    ))}
