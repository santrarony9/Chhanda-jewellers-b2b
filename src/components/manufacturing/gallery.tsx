                ...
                <div className="grid grid-cols-1 md:grid-cols-4 grid-rows-3 gap-4 h-[800px] md:h-[600px]">
                    {galleryItems.map((item, i) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className={`${item.span} relative rounded-xl overflow-hidden group border border-surface-light`}
                        >
                            <div className={`absolute inset-0 ${item.color} transition-opacity opacity-50 group-hover:opacity-70`} />
